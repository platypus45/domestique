...
- Diagnostics no longer warn three times per start that the workout classification is stale and ask you to run a developer script the app does not ship. The check compared file timestamps that no installer preserves, so every install tripped it; it now checks that each workout in the folder has a classification, and says so once, in plain words, only when one is missing.
...
- Diagnostics now report how many upcoming days have no workout attached.
- **A missed hard session is now registered, re-owed where it safely fits, and explained — even across the week boundary.** Plans keep their own week rhythm (a plan generated on a Friday runs Friday to Thursday). A session missed on the last day of such a week was never marked as missed on the next day, so nothing re-fitted the week, nothing was rescheduled, and the calendar showed the missed day without a word about what became of it. Every recent past day is now reconciled regardless of week boundaries, and a hard session missed in the last six days is owed to the current week the same way as one missed inside it. What it can do with the lost load follows the evidence already in the app: the hard session itself is only re-owed to a day at least 48 hours from any other hard day (including one you actually rode last week) and within the weekly hard-session cap; the remaining load is recycled as easy volume, spread over several days, never past a day's availability and never more than half a session longer; and only what your fitness allows is carried into next week (within the acute-to-chronic load ceiling and the ramp rate, never into a recovery week or a taper, never when form is already low). Whatever cannot be placed is dropped, and the missed day's card says exactly why — for example "80 TSS dropped — no day at least 48 h from a hard session (Sat 12 Sep vo2max); no room within your availability". Days that received recycled minutes say where they came from.
- **Imported rides can reach intervals.icu again.** Sending an imported ride file to intervals.icu needs an upload permission that the sign-in never asked for, so every upload failed silently in the background. The sign-in now asks for it; if you connected before this version, the app tells you after an import to reconnect (Settings → Connections) to grant it, until you do. The import itself was never affected.
...
- **If a sign-in still fails on a certificate error, the log now names the issuer** of the certificate the app was shown, so the next report points at the antivirus or proxy instead of leaving everyone guessing. The Diagnostics panel shows which verifier is in use.
...
- **Fixed: intervals.icu sign-in failed on Windows machines where HTTPS is inspected.** The rider's log showed it: the sign-in's final step verified the intervals.icu certificate against the app's own certificate list only, so an antivirus with "HTTPS scanning" (Kaspersky, ESET, Avast and others) or a corporate proxy — which re-sign every connection with their own root certificate — made that step fail with a certificate error, while the browser and the ride sync (which trust the Windows certificate store) worked fine. The sign-in now trusts both the app's list and the operating system's store, on every platform.
...
- **The 20-minute and 60-minute tests are now free rides.** The measured blocks carried a power target of 100% — so in erg mode the trainer held you at your *old* FTP during the very effort meant to measure your new one. Those blocks (and the 5-minute all-out clearing effort) now have no target at all: ride the hardest steady pace you can hold, exactly as the protocol intends. The FIT download had a second version of the same mistake: a free-ride block was exported as a 50%-of-FTP target, so a head unit would have told you to ride the test at half power — it is now an open step with no target. Recognition and the calculation are unchanged.
- **Your protocol choice sticks.** Picking the ramp or the 60-minute test on a test day used to snap back to "20-minute · selected" after closing and reopening the day, and the preview underneath could show the previous file's name, duration and TSS beneath the new file's chart. The choice now survives every plan re-fit, the badge follows the file that is actually attached, and the panel refreshes on switch.
- **Imported ride files were being read as zero power, zero heart rate, zero cadence — since the very first release.** The importer asked the FIT library for each value through a method that library has never had, and every call was wrapped in a silent fallback to zero. Anyone importing FIT files directly (rather than syncing from intervals.icu) got empty ride stats, no heart-rate drift, no DFA, and never an FTP-test suggestion from an imported test. Found by tracing the new analysis view end-to-end on a synthetic ride file; fixed at the source and pinned by a test that imports a real FIT and checks the numbers. Rides you imported earlier are re-read automatically the next time the app lists them — their stats fill in, and any effort rating you gave is kept.
- **After a test, you see the evidence, not just a number.** The review screen now walks through Recognise → Measure → Compute → Apply: which protocol was recognised and from what, the power trace with the scored window shaded (the 20-minute block, the hour plateau, or the best ramp minute), the 5-minute clearing effort marked, the ramp halt point if there was one, the exact factor applied, your old FTP against the suggestion, and every caveat that applies — missing clearing effort, pacing drift, a ramp over-read for punchy riders, the watts-per-kilo band. Then the same Update / Keep / Custom choice.
- **Two riders on one computer: screened again.** With multiple profiles on the same install, every profile's rides, FTP-test suggestions, accept/decline answers, plan choices and custom workout folder stay in that profile — re-verified end to end with new tests. Two small gaps closed: the Diagnostics panel could show the previous profile's library status for up to a minute after switching, and the browser's "already showed you this FTP prompt" memory wasn't profile-aware.
- **Scheduling a test is a two-step screen.** Pick the day on a real month calendar, then pick the protocol from the same three cards you see on the day itself — choosing the card schedules it. The day before is eased for fresh legs and the plan re-fits around it, as before.
...
- **A stale custom workout-folder setting no longer empties your library.** If you ever pointed the app at your own workout folder and that folder later moved or vanished, the app silently planned from nothing. It now ignores the dead setting, logs why, and falls back to the built-in library of 4,300 workouts.
- **The calendar is honest about empty days.** A planned session that has no workout attached now says "no workout matched" instead of posing as an innocent-looking training card.
- **One click tells you if this ever happens again.** The Diagnostics panel now reports the live health of the workout library — how many workouts loaded, whether the hard-session pools are populated, and the exact reason if anything is wrong.
- **Every release is now proven to ship its library.** The macOS, Windows and Linux build pipelines each launch the actual packaged app and verify the full workout library and its classification data survived packaging — the failure class behind this report (and an older incident) can no longer reach a download.
- Under the hood, the repository was reorganised into a much tidier layout. Nothing changes on your machine: your data, settings, plans and sign-in all live in your own user folder and are untouched, and the packaged app was verified byte-for-byte identical before and after the reorganisation.

## v3.11.0 — Pick your FTP test, and trust the number (2026-08-30)

- **Pick your FTP test — and see the evidence for each.** A test day now offers three protocols side by side: the 20-minute test (the default — the best-validated in the literature, with the full protocol including the 5-minute clearing effort that Zwift-style tests skip; leaving it out inflates your number by about 5%), the ramp (quick and pacing-free, with the honest caveat printed on the card: its 0.75 factor has never been validated in a peer-reviewed study and punchy riders read high), and a proper 60-minute hour of power (the definitional test — no correction factor at all). Which factor fits *you* also depends on your level: when your weight is set, the review screen shows the range measured for riders at your watts-per-kilo next to the default. The full review of what each protocol can and cannot claim is in [the science documentation](docs/SCIENCE.md#2a-the-ftp-test-protocols--what-the-literature-actually-supports).
- **Test day, when you want it.** A "Schedule FTP test" button on the calendar puts a test on any day you pick — the day before is automatically eased so you arrive on fresh legs, the rest of the plan re-fits around it, and it politely refuses a day you have already ridden hard. Scheduled tests also stopped silently skipping: a test that collided with a recovery week used to vanish for a whole extra cycle; now it simply moves to the next normal week. Rolling plans get their first test in week two instead of whenever the cadence happened to land.
- **Tests ridden through intervals.icu now count too.** Ride your test anywhere — the app recognises it when the ride syncs in, computes your number, and shows the same Accept / Keep / Edit review. Your answer is remembered: no re-asking after every sync, and a suggestion no longer quietly disappears half an hour after it appeared.
- **The number you accept is computed more carefully.** Rides recorded with "smart recording" (many Garmin and Wahoo units) used to have their test windows silently stretched — a "20-minute" average could span far more than 20 minutes and read low. An abandoned hour of power is scored by the validated 20-minute method instead of a blind hour average, and the review screen says so. A finished test is no longer graded "overdid it" by the session scorer for being exactly as hard as a maximal test is supposed to be.
- **Test recognition was made hard to fool — by trying hard to fool it.** An adversarial review threw ordinary rides at the detector until it broke, then every break was fixed: a 2×20 interval session, a ride finishing on a steady climb, and a long progressive ride could each have been mistaken for a test and produced a confident wrong FTP; a ride merely *titled* like a test could be scored as one; corrupted power samples could inflate a suggestion by half. None of these survive: recognition is exact when you ride the provided test workouts, deliberately silent on anything ambiguous, and a suggested number outside physiological range is refused outright.
- **The Workout Picker respects the duration you set.** Asking for 30 minutes used to return workouts of an hour or more: the search window stretched from 0 to 60 minutes and the ranking favoured bigger sessions, which are the longer ones. The window now scales with what you asked for, and results are ordered by how close they are to your requested time.
...
- **The summary's intensity-distribution and polarization charts now fill in for synced rides.** Zone-by-zone riding time was only read from imported files, so riders whose data arrives from intervals.icu saw empty charts over real riding.
- **A week going wrong is noticed while it is still going wrong.** Absence detection only looked at completed weeks, so going dark on a Monday could not trigger the plan's rebuild until the following Monday. The current week now counts, in proportion to how much of it has passed — from day four, and never on its own: a rebuild still requires a fully missed week before it.
