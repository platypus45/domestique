...
- **Imported rides can reach intervals.icu again.** Sending an imported ride file to intervals.icu needs an upload permission that the sign-in never asked for, so every upload failed silently in the background. The sign-in now asks for it; if you connected before this version, the app tells you once, after your next import, to reconnect (Settings → Connections) to grant it. The import itself was never affected.
