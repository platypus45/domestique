...
**Why block grading is not shipped (v3.6.0).** The three execution axes cannot see structure — stopping after 10 of 13 reps survives TSS and time-in-zone nearly intact — so "which prescribed blocks did I do?" is worth answering. It is not answerable from a lap list. Laps carry no labels, and a head unit types a lap "work" on intensity alone, so warm-up ramp steps, fast-pedal drills and above-threshold float legs all arrive looking like prescribed work. Four successive attempts to infer the mapping — positional, shape-matched, anchored on the ride clock, then gated on a decidability test derived from the workout file — each survived one adversarial review and failed the next, in a different way each time: certifying a session the rider abandoned as complete, grading a block ridden *harder* than prescribed as not ridden, and naming a fully completed block "missing" when the recoveries ran long. Measured blast radius at the last iteration: 61% of sessions misreported when ridden 20 points of FTP above target, 39% when each recovery ran three minutes long.

The grader and its measurement harness stay in the tree unsurfaced. It returns when it is driven by real lap timestamps rather than inferred from block shape — intervals.icu exposes them on the raw activity; they are simply not persisted yet. Until then a rider sees the execution score, which is true.
