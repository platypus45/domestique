...
**Block grading is decidable or it is silent (v3.6.0).** Laps are unlabelled, and a head unit types a lap "work" on intensity alone — so a warm-up ramp step, a fast-pedal drill or an above-threshold float leg all arrive looking like prescribed work. When a session contains such a block with the same shape as a real interval, sitting near the same part of the ride, the mapping from laps to blocks is genuinely under-determined: no inference distinguishes "that lap was the block" from "the block was skipped and that lap was the warm-up". Three successive attempts to infer it anyway — positional, then shape-matched, then anchored on the ride clock — each produced confident wrong verdicts on hundreds of library workouts, the worst telling a rider who abandoned an entire set that they had completed every block and 100% of the prescribed work. The shipped rule decides gradeability from the workout file itself, before the ride is considered, and returns nothing for the ~19% of interval sessions whose own structure makes the question unanswerable. A missing verdict is recoverable; a false one is not.
