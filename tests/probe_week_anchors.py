...
def _starts(weeks):
    """Week starts, excluding the taper.

    The taper is deliberately off the Monday grid: it is laid backward from
    race day, and anchoring it leaves the race week only the days between the
    last Monday and the event -- measured, that cut a race week from three rest
    days to zero. So it is not a finding here, it is the design.
    """
    return [w.start for w in weeks if getattr(w, "phase", "") != "taper"]
