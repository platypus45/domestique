...
class TestPlanLevelInvariants(unittest.TestCase):
    ...
    def test_healthy_volume_not_starved(self):
        # A healthy-volume plan must reach each peak week's effective cap (F3 must
        # not UNDER-fill) AND not be starved to 1 HIT. The exact "high ceiling →
        # hit_count_max=3" is covered deterministically by TestEffectiveHitMaxUnit;
        # end-to-end the peak ceiling is target-CTL-bound, so assert the robust
        # invariant: realized == effective cap, and ≥2 in peak.
        peak = [w for w in self._build_weeks(700, 0)
                if w.phase == "peak" and not getattr(w, "is_stepback", False)]
        if not peak:
            self.skipTest("no peak phase in this plan")
        reached = max(tp._week_hit_count(w) for w in peak)
        target = max(tp._effective_hit_max(w) for w in peak)
        self.assertEqual(reached, target,
                         f"healthy peak under-fills its effective cap ({reached} < {target})")
        self.assertGreaterEqual(reached, 2, "healthy peak starved below 2 HIT")
